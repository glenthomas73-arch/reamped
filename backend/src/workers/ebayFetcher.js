                                    ...
                                    timeout: 15000,
                                        ...
                                        function normalize(item, catName) {
                                          const price = item.price ? parseFloat(item.price.value) : 0;
                                            const shipping = item.shippingOptions?.[0]?.shippingCost?.value ? parseFloat(item.shippingOptions[0].shippingCost.value) : null;
                                              return {
                                                  external_id: item.itemId, platform: 'ebay', title: item.title,
                                                      price, currency: item.price?.currency || 'USD',
                                                          condition: normalizeCondition(item.conditionId), category: catName,
                                                              brand: item.brand || null, model: null,
                                                                  location_city: item.itemLocation?.city || null, location_country: item.itemLocation?.country || 'US',
                                                                      shipping_available: true, shipping_cost: shipping,
                                                                          listing_url: item.itemWebUrl, affiliate_url: item.itemWebUrl ? `${item.itemWebUrl}&utm_source=reamped` : null,
                                                                              image_urls: item.image ? [item.image.imageUrl] : [],
                                                                                  seller_name: item.seller?.username || null,
                                                                                      seller_rating: item.seller?.feedbackPercentage ? parseFloat(item.seller.feedbackPercentage) / 100 : null,
                                                                                          seller_reviews: item.seller?.feedbackScore || 0, listed_at: item.itemCreationDate || null,
                                                                                            };
                                                                                            }
                                                                                                                                                        ...
                                                                                                                                                        l.listing_url, l.affiliate_url, JSON.stringify(l.image_urls),
                                                                                                                                                                l.seller_name, l.seller_rating, l.seller_reviews, l.listed_at]);
                                                                                                                                                                  ...
                                                                                                                                                                  async function runEbayFetcher() {
                                                                                                                                                                    logger.info('Starting eBay fetch cycle...');
                                                                                                                                                                      const token = await getToken();
                                                                                                                                                                        let total = 0;
                                                                                                                                                                          for (const cat of CATEGORIES) {
                                                                                                                                                                              try {
                                                                                                                                                                                    const data = await fetchCategory(token, cat.id);
                                                                                                                                                                                          const listings = (data.itemSummaries || []).map(i => normalize(i, cat.name));
                                                                                                                                                                                                if (listings.length) {
                                                                                                                                                                                                                ...
                                                                                                                                                                                                                await scoreListings('ebay', listings.map(l => l.external_id));
                                                                                                                                                                                                                        total += listings.length;
                                                                                                                                                                                                                                logger.info(`eBay [${cat.name}]: ${listings.length} listings`);
                                                                                                                                                                                                                                          ...
                                                                                                                                                                                                                                          } catch (err) { logger.error(`eBay [${cat.id}] error: ${err.message}`); }
